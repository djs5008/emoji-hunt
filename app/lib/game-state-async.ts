...
/**
 * Updates the game state of a lobby
 * 
 * @description Simple utility to change the game state. Used internally by
 * other state management functions. No validation is performed.
 * 
 * @param {string} lobbyId - The lobby to update
 * @param {Lobby['gameState']} state - The new game state
 */
 ...
 * 
 * @description Handles round progression after a round ends. If all 5 rounds
 * are complete, transitions to 'finished' state. Otherwise, starts the countdown
 * for the next round.
 * 
 * @param {string} lobbyId - The lobby to advance
 * 
 * Game flow:
 * - Rounds 1-4: Start countdown for next round
 * - Round 5: Transition to finished state
 */
